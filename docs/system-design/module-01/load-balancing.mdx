...
<InterviewQ>
  ...
  <InterviewQItem>
    ...
    <InterviewAnswer>
        ...
        When a server is added or removed, only about <Code>1/N</Code> of keys need to be remapped, unlike naive <Code>hash % N</Code> where almost everything remaps. 
  ...
  <InterviewAnswer>
    ...
    Health checks, such as active probes like <Code>/health</Code> and passive checks that monitor real traffic, let the load balancer detect failed servers and remove them from the pool until they recover.
  </InterviewAnswer>
</InterviewQItem>
...
<InterviewQItem>
  ...
  <InterviewAnswer>
    ...
    <Code>GSLB (DNS) → Tokyo LB → healthy Tokyo server</Code>
  </InterviewAnswer>
</InterviewQItem>

<InterviewQItem>
  <InterviewQuestion>
    #### 9. Is the load balancer itself a single point of failure?
  </InterviewQuestion>
...
<InterviewQItem>
  ...
  <InterviewAnswer>
    ...
    1. A smart client library that maintains a list of healthy instances, such as Eureka or gRPC client-side load balancing.
    2. Sidecar proxies like Envoy, where every pod has its own proxy that handles load balancing, retries, and circuit breaking.
    3. A central load balancer for each service.
