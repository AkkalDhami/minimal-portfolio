...
export function SqlPlayground({ docs = false, query }: SqlPlaygroundProps) {
  ...
  const { status, initError, execute } = useSqlDatabase();
  const [playError] = useSound(back004Sound);
  const [playSuccess] = useSound(successChimeSound);
  const [selectedQueryId, setSelectedQueryId] = useState(SAMPLE_QUERIES[0].id);
  const [sql, setSql] = useState(query ?? DEFAULT_QUERY);
